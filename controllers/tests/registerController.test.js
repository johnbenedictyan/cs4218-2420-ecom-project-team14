...
describe("Register Controller Tests", () => {
  ...
  it("should not allow user with non-empty name of length 151 to be registered", async () => {
    req.body.name =
      "John William Samuel Douglas Russell Wallace Brandon Blaine James Joseph Johnson Monrole Jefferson Theodore Timothy Reece Franklin Charles Watson Holmes";
  ...
  it("should not allow user with non-empty invalid address of length 151 to be registered", async () => {
    req.body.address =
      "This is an extremely long long address with more than one hundred and fifty characters and this should not be allowed when trying to update the profile";
  ...
  it("should throw an error when the user faces an error while trying to register", async () => {
    userModel.findOne = jest.fn().mockImplementation(() => {
      throw new Error(
        "Error in connecting with database to check for existing user"
      );
    });
