...
describe("Product Filter Integration Tests", () => {
  ...
  describe("Product Filters Controller Integration Tests", () => {
    it("should not filter out any products if no filters are provided (Pairwise)", async () => {
    ...
    // Test 1: Filter by single category (Just above boundary)
    it("should filter products by a single category (Pairwise/Boundary)", async () => {
      const response = await request(app)
        .post(`${productPath}/product-filters`)
        .send({
    ...
    // Test 2: Filter by all categories (only) should return all products
    it("should filter products by multiple categories (Equivalence)", async () => {
      const response = await request(app)
        .post(`${productPath}/product-filters`)
        .send({
    ...
    // Test 3: Filter by price range only
    it("should filter products by price range (Equivalence)", async () => {
      const response = await request(app)
        .post(`${productPath}/product-filters`)
        .send({
          checked: [],
    ...
    // Test 4: Filter by price range (Boundary Value of 5000000 should only and only return that product)
    it("should filter products by both category and price range (Boundary)", async () => {
      const response = await request(app)
        .post(`${productPath}/product-filters`)
        .send({
          checked: [],
    ...
    // Test 5: Filter by both category and price range
    it("should filter products by both category and price range (Pairwise)", async () => {
      const response = await request(app)
        .post(`${productPath}/product-filters`)
        .send({
          checked: [categoryId1.toString()],
    ...
    // Test 6: Invalid category ID format should return error (Negative)
    ...
    // Test 7: Invalid price range format should return error (Negative)
    ...
    // Test 8: Non-numeric values in price range should return error (Negative)
    ...
    // Test 9: No matching products should return empty array
